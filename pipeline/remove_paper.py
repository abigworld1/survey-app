...
例:
  python -m pipeline.remove_paper --arxiv 1901.11282
  python -m pipeline.remove_paper --slug foo                    # ファイル名(拡張子なし)で
  python -m pipeline.remove_paper --list                        # MAPFの一覧を表示
...
def main(argv=None):
    ...
    for key in targets:
        ...
        if info.get("file") and os.path.exists(path):
            os.remove(path)
        path_en = os.path.join(ROOT, info.get("file_en", ""))
        if info.get("file_en") and os.path.exists(path_en):
            os.remove(path_en)
